...
class MixinDatasetTest(MixinDatasetProtocolTest):
    def test_dataset__len__(self, dataset: Dataset[Any], samples: list[Sample]) -> None:
    ...
    def test_dataset_select(self, dataset: Dataset[Any]) -> None:
        type(self).assert_select(dataset)

    def test_dataset_slice(self, dataset: Dataset[Any]) -> None:
        type(self).assert_slice(dataset)

    def test_dataset_sample(self, dataset: Dataset[Any], samples: list[Sample]) -> None:
        type(self).assert_sample(dataset, samples)

    def test_dataset_get(self, dataset: Dataset[Any]) -> None:
        type(self).assert_get(dataset)

    def test_dataset__add__(self, dataset: Dataset[Any]) -> None:
