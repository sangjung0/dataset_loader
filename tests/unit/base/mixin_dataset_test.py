...
class MixinDatasetTest(MixinDatasetProtocolTest):
    def test_dataset__len__(
        self, dataset: Dataset[Any, Any], samples: list[Sample]
    ) -> None:
        type(self).assert__len__(dataset, samples)

    def test_dataset__iter__(
        self, dataset: Dataset[Any, Any], samples: list[Sample]
    ) -> None:
        type(self).assert__iter__(dataset, samples)

    def test_dataset__getitem__(
        self, dataset: Dataset[Any, Any], samples: list[Sample]
    ) -> None:
        type(self).assert__getitem__(dataset, samples)

    def test_dataset_select(self, dataset: Dataset[Any, Any]) -> None:
        type(self).assert_select(dataset)

    def test_dataset_slice(self, dataset: Dataset[Any, Any]) -> None:
    ...
    def test_dataset_get(self, dataset: Dataset[Any, Any]) -> None:
        type(self).assert_get(dataset)

    def test_dataset__add__(self, dataset: Dataset[Any, Any]) -> None:
    ...
    def test_dataset_to_config_and_from_config(
        ...
        type(self).assert_to_config_and_from_config(
            dataset, samples, Dataset  # type:ignore[type-abstract]
        )

    def test_dataset_sample_identity(self, dataset: Dataset[Any, Any]) -> None:
