...
class MixinIRDatasetTest(MixinDatasetProtocolTest):
    ...
    def test_ir_thread_iter(self, ir_dataset: IRDataset) -> None:
    ...
    def test_ir__len__(self, ir_dataset: IRDataset, ir_samples: list[IRSample]) -> None:
    ...
    def test_ir_select(self, ir_dataset: IRDataset) -> None:
        type(self).assert_select(ir_dataset)

    def test_ir_slice(self, ir_dataset: IRDataset) -> None:
        type(self).assert_slice(ir_dataset)

    def test_ir_sample(self, ir_dataset: IRDataset, ir_samples: list[IRSample]) -> None:
        type(self).assert_sample(ir_dataset, ir_samples)

    def test_ir_get(self, ir_dataset: IRDataset) -> None:
        type(self).assert_get(ir_dataset)

    def test_ir__add__(self, ir_dataset: IRDataset) -> None:
    ...
    def test_ir_sample_identity(self, ir_dataset: IRDataset) -> None:
