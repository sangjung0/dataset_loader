...
class MixinIRDatasetTest(MixinDatasetProtocolTest, Generic[LabelT]):
    """
    Need: ir_dataset, samples
    """

    def test_ir_thread_iter(self, ir_dataset: IRDataset[LabelT]) -> None:
    ...
    def test_ir__len__(
        self, ir_dataset: IRDataset[LabelT], ir_samples: list[IRSample[LabelT]]
    ) -> None:
    ...
    def test_ir__getitem__(
        self, ir_dataset: IRDataset[LabelT], ir_samples: list[IRSample[LabelT]]
    ) -> None:
        type(self).assert__getitem__(ir_dataset, ir_samples)

    def test_ir_select(self, ir_dataset: IRDataset[LabelT]) -> None:
        type(self).assert_select(ir_dataset)

    def test_ir_slice(self, ir_dataset: IRDataset[LabelT]) -> None:
    ...
    def test_ir_get(self, ir_dataset: IRDataset[LabelT]) -> None:
        type(self).assert_get(ir_dataset)

    def test_ir__add__(self, ir_dataset: IRDataset[LabelT]) -> None:
        type(self).assert__add__(ir_dataset, IRDataset[LabelT])
    ...
    def test_ir_to_pointer_and_from_pointer(
        ...
        type(self).assert_to_config_and_from_config(
            ir_dataset, ir_samples, IRDataset[LabelT]
        )

    def test_ir_sample_identity(self, ir_dataset: IRDataset[LabelT]) -> None:
